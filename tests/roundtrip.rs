...
async fn replicate_then_restore_roundtrip() {
    ...
    let v1 = LocalVersion {
        ...
        dirs: vec![], journals: vec![], pointers: vec![],
    };
...
async fn immutable_files_upload_once() {
    ...
    let v1 = LocalVersion { seqno: 1, parent: None, files: vec![f.clone()], dirs: vec![], journals: vec![], pointers: vec![] };
    let v2 = LocalVersion { seqno: 2, parent: Some(1), files: vec![f.clone()], dirs: vec![], journals: vec![], pointers: vec![] };
